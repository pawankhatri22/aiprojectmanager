...
import com.aiprojectmanager.meeting.ZoomApiService;
...
public class SessionService {
    ...
    public SessionService(SessionRepository sessionRepository, CurrentUserService currentUserService, UserRepository userRepository,
                          MentorProfileRepository mentorProfileRepository, ZoomApiService zoomApiService, NotificationService notificationService) {
    ...
    public SessionDtos.SessionResponse request(SessionDtos.RequestSessionRequest req) {
        ...
        validateMentorAvailability(mentor.getId(), req.scheduledTime(), req.durationMinutes(), null);
    ...
    private SessionDtos.SessionResponse updateStatus(Long id, SessionStatus status, Role requiredRole) {
        ...
        if (status == SessionStatus.PENDING_PAYMENT) {
            ...
            notificationService.notify(s.getGraduate(), NotificationType.APPROVAL, "Session #" + s.getId() + " approved by mentor.");
            notificationService.notify(s.getGraduate(), NotificationType.PAYMENT_PENDING, "Payment pending for session #" + s.getId());
        }
        ...
        if (status == SessionStatus.PAID) {
            ...
            validateMentorAvailability(s.getMentor().getId(), s.getScheduledTime(), s.getDurationMinutes(), s.getId());
            String meetingLink = zoomApiService.createMeeting(s);
            s.setMeetingLink(meetingLink != null ? meetingLink : "Creating Meeting link taking time, will send link on your email soon!");
            notificationService.notify(s.getGraduate(), NotificationType.PAYMENT_SUCCESS, "Payment successful for session #" + s.getId());
            notificationService.notify(s.getMentor(), NotificationType.PAYMENT_SUCCESS, "Graduate paid for session #" + s.getId());
        }
    ...
    public void validateMentorAvailability(Long mentorId, LocalDateTime start, Integer duration, Long ignoreSessionId) {
