...
export interface SessionItem {
  ...
  price: number;
  status: 'REQUESTED' | 'PENDING_PAYMENT' | 'PAID' | 'REJECTED' | 'CANCELLED' | 'COMPLETED';
  confirmationLink?: string;
  meetingLink?: string;
}
export interface NotificationItem { id: number; type: string; message: string; read: boolean; createdAt: string; }
export interface RescheduleItem { id: number; sessionId: number; requestedBy: number; oldScheduledTime: string; oldDurationMinutes: number; newScheduledTime: string; newDurationMinutes: number; reason: string; status: 'REQUESTED'|'ACCEPTED'|'REJECTED'; createdAt: string; decidedAt?: string; }
export interface AttendanceItem { id: number; sessionId: number; participantId: number; participantEmail: string; joinTime?: string; leaveTime?: string; status: string; updatedAt: string; }
